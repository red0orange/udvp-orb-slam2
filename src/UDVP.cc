...
class UDVPUnconstrainedEdge: public UDVPEdge  // 无约束情况下的Edge
{
public:
    UDVPUnconstrainedEdge( double fx, double fy, double cx, double cy, double fov, double max_depth,
            Frame* cur_frame,
            MapPoint* map_point,
            double phi, double M, double N, double N_MAX 
            ): UDVPEdge(fx, fy, cx, cy, fov, max_depth, cur_frame, map_point, phi, M, N, N_MAX)
    {}
    // 计算曲线模型误差
    void computeError()
    {
...
cv::Mat UDVP::optimization(Frame* cur_frame, std::vector<MapPoint*> local_map_points){
    ...
    typedef UDVPUnconstrainedEdge Edge;
    // 构造优化图
    vector<Edge*> vpEdgesMono;
    vector<size_t> vnIndexEdgeMono;
    double tmp_phi = 0;
    for (int i = 0; i < local_map_points.size(); i++)
    {
        // UDVPEdge* e = new UDVPEdge(fx, fy, cx, cy, fov, max_depth, cur_frame, local_map_points[i], tmp_phi, M, N, N_MAX);
        Edge* e = new Edge(fx, fy, cx, cy, fov, max_depth, cur_frame, local_map_points[i], tmp_phi, M, N, N_MAX);
